...
def contar_entradas_matriz(matriz):
    """
    Cuenta cuántas entradas en una matriz son iguales a 1, cuántas son diferentes a 1,
    y calcula el porcentaje de cada tipo.

    :param matriz: Lista de listas que representa una matriz.
    :return: Un diccionario con el conteo y porcentaje de entradas iguales a 1 y diferentes a 1.
    """
    # Total de elementos en la matriz
    total_elementos = sum(len(fila) for fila in matriz)

    # Contar elementos iguales a 1 y diferentes a 1
    iguales_a_1 = sum(1 for fila in matriz for x in fila if x == 1)
    diferentes_a_1 = total_elementos - iguales_a_1  # Alternativa a contar los diferentes

    # Calcular porcentajes
    porcentaje_iguales_a_1 = (iguales_a_1 / total_elementos) * 100 if total_elementos > 0 else 0
    porcentaje_diferentes_a_1 = (diferentes_a_1 / total_elementos) * 100 if total_elementos > 0 else 0

    return {
        "iguales_a_1": iguales_a_1,
        "diferentes_a_1": diferentes_a_1,
        "porcentaje_iguales_a_1": porcentaje_iguales_a_1,
        "porcentaje_diferentes_a_1": porcentaje_diferentes_a_1
    }

resultado = contar_entradas_matriz(p_rho)
print(resultado)

filas, columnas = p_rho.shape
print(f"Filas: {filas}, Columnas: {columnas}")  # Salida: Filas: 3, Columnas: 3


# region No se si son fnciones del paquete o solo para comparar
# ########################

# Plano Principal y Círculo de Correlación directamente de UMAP para comparar

# Función para calcular las componentes UMAP
def calculate_umap_components(data, n_neighbors=cantidad_vecinos, min_dist=0.1, metric='euclidean', n_components=2):
    """
    Calculate the first two components using UMAP.

    Parameters:
    data (numpy array): The input data.
    n_neighbors (int): Number of neighbors used in UMAP.
    min_dist (float): Minimum distance between embedded points.
    metric (str): Distance metric to use in UMAP.
    n_components (int): Number of dimensions to reduce to (default is 2 for this function).

    Returns:
    components (numpy array): The UMAP components.
    """
...
# Función para calcular la correlación Riemanniana entre las variables originales y las dos primeras componentes UMAP
def correlation_riemannian_umap_columns(data, umap_components, rho):
    """
    Calculate Riemannian correlations between the columns of the data and each UMAP component.

    Parameters:
    data (numpy array): The original data.
    umap_components (numpy array): The UMAP components (2D array).
    rho (numpy array): The matrix of Riemannian distances between the data points.

    Returns:
    correlations (pandas DataFrame): A DataFrame with two columns: the correlation of the first UMAP component with each variable in `data`,
                                     and the correlation of the second UMAP component with each variable in `data`.
   """
    # Combine original data and UMAP components into one DataFrame
    combined_data = pd.DataFrame(np.hstack((data, umap_components[:, 0:2])),
                                 columns=[f'feature_{i+1}' for i in range(data.shape[1])] + ['UMAP_Component_1', 'UMAP_Component_2'])

    # Calculate the Riemannian covariance matrix for the combined data
    cov_matrix_riemannian = covariance_matrix_riemannian_with_riemannian_mean(combined_data, rho)

    # Initialize a DataFrame to store the correlations
    correlations = pd.DataFrame(index=[f'feature_{i+1}' for i in range(data.shape[1])],
                                columns=['UMAP_Component_1', 'UMAP_Component_2'])

    # Calculate the correlations for the first UMAP component
    for i in range(data.shape[1]):  # Loop through original data columns
        correlations.loc[f'feature_{i+1}', 'UMAP_Component_1'] = cov_matrix_riemannian[i, -2] / np.sqrt(cov_matrix_riemannian[i, i] * cov_matrix_riemannian[-2, -2])

    # Calculate the correlations for the second UMAP component
    for i in range(data.shape[1]):  # Loop through original data columns
        correlations.loc[f'feature_{i+1}', 'UMAP_Component_2'] = cov_matrix_riemannian[i, -1] / np.sqrt(cov_matrix_riemannian[i, i] * cov_matrix_riemannian[-1, -1])

    return correlations


# Función para calcular la correlación clásica entre las variables originales y las dos primeras componentes UMAP
...
# Calcula las componentes UMAP
umap_components = calculate_umap_components(data)
umap_components

# Calcula las correlaciones Riemannianas entre las variables originales y las dos primeras componentes UMAP
riemannian_correlations = correlation_riemannian_umap_columns(data, umap_components, rho)

# Muestra las correlaciones resultantes
print("Riemannian Correlation Matrix (first and second UMAP components):")
print(riemannian_correlations)

# Calcula las correlaciones clásicas entre las variables originales y las dos primeras componentes UMAP
classic_correlations = correlation_clasica_umap_columns(data, umap_components)
